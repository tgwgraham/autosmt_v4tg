...
def write_parent_macro(settings,basefname,macroname):
    with open(macroname, 'w') as fh:
        fh.write(MACROSTRING_0)
        if settings['macro_settings']['extraOC']:
...
def movefile(basefname,snapnum,ms):    
        ...
        if snapnum==4:
            for k in ms['extraOCs'].keys():
                shutil.move(basefname + f'temp/snap_{k}.tif', 
                        basefname + f'output/snaps2_{k}/{maxint + 1}.tif')
        else:
...
# Take images in an extra OC
    extraOCmac = rf"""// TAKE AN IMAGE WITH AN EXTRA OC
    ...
    if not bool(large_regions):
      ...
      with open(f'{basefname}/temp/movethere.mac', 'w') as fh:
        if extraOC:
            ...
            for k in ms['extraOCs'].keys():
                fh.writelines(extraOCmac.format(current_OC=ms['extraOCs'][k]['extraOC'],channel_name=k)) 
            fh.writelines(done4mac)
